...
Evidence reviewed 2026-09-05. Scope: reports explicitly tied to Ignition 8.1.40–8.1.53. Use only for an observed matching failure or an explicit version-compatibility question. Use the supplied Ignition patch and affected Designer/Workstation/browser environment when available; the runner's `0.3.x` version is separate. Missing live access does not block offline authoring. Record relevant assumptions and put target checks in `VALIDATION.md`. A project ZIP does not upgrade Ignition or its client environment.
...
**Action:** preserve the supplied source and normal resource backup/drift protections, reopen the changed view, and verify pipe-to-tag assignments against known states alongside the existing geometry check. Limit corrections to the affected mappings; do not claim a patch remedy without matching evidence.
...
**Action:** for a matching scripting failure, record the build and initialization evidence. If version remediation is authorized, evaluate the supported release containing the rollback and verify affected scripting/page behavior. A portable page package must not silently introduce an Ignition upgrade requirement; report relevant version dependencies explicitly.
...
Select checks for the features actually used and the symptom being addressed. These release notes do not require a full upgrade matrix, device lab, or repeated regression suite for every new visualization. Once the required offline checks pass, deliver the package with relevant target checks clearly marked as completed or pending.
